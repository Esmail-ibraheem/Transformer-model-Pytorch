...
class PositionalEncodingLayer(nn.Module):
    def __init__(self, d_model: int, sequence_length: int, dropout: float) -> None:
        ...
        deivation_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))

        PE[:, 0::2] = torch.sin(Position * deivation_term)
        PE[:, 1::2] = torch.cos(Position * deivation_term)
        PE = PE.unsqueeze(0)
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def forward(self, query, key, value, mask):
        ...
        x = x.transpose(1,2).contiguous().view(x.ahspe[0], -1, self.heads * self.d_k)
        return self.W_O(x)
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, Encoder_output, source_mask, target_mask):
        ...
        x = self.residual_connection[1](x, lambda x: self.decoder_cross_attention_block(x, Encoder_output, Encoder_output, target_mask)) # x, Encoder_output, Encoder_output, target_mask = Query, Key, Value, target_mask
...
class LinearLayer(nn.Module):
    def __init__(self, d_model: int, vocab_size: int) -> None:
        super().__init__()
        self.Linear = nn.Linear(d_model, vocab_size) # convert d_model (embeddings) to word again
    def forward(self, x):
...
def Transformer_model(source_vocab_size: int, target_vocab_size: int, source_sequence_length: int, target_sequence_length: int, d_model: int = 512, Layers: int = 6, heads: int = 8, dropout: float = 0.1, d_ff: int = 2048)->TransformerBlock:
    ...
    for _ in Layers:
        ...
        decoder_corss_attention_block = MultiHeadAttentionBlock(d_model, heads, dropout)
        decoder_feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout)
        decoder_block = DecoderBlock(decoder_self_attention_block, decoder_corss_attention_block, decoder_feed_forward_block)
